...
if (isset($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        ...
        mysqli_query($conn, "UPDATE technicians SET id = (@count:=@count+1) ORDER BY id");
        mysqli_query($conn, "ALTER TABLE technicians AUTO_INCREMENT = 1");

        echo "<script>
            alert('Technician deleted successfully! ID reordered.');
