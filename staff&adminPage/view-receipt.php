        ...
        <div class="row g-3">
          <div class="col-auto">
            <div class="position-relative">
              <input id="searchInput" class="form-control px-5" type="search" placeholder="Search Transaction ID" onkeyup="searchTransaction()">
          ...
          <div class="col-auto flex-grow-1 overflow-auto">
            ...
            <div class="btn-group position-static">
              ...
              <ul class="dropdown-menu">
                  <li><a class="dropdown-item" href="javascript:;" onclick="filterByDate('today')">Today</a></li>
                  <li><a class="dropdown-item" href="javascript:;" onclick="filterByDate('week')">This Week</a></li>
                  <li><a class="dropdown-item" href="javascript:;" onclick="filterByDate('month')">This Month</a></li>
                  <li><hr class="dropdown-divider"></li>
                  <li><a class="dropdown-item" href="javascript:;" onclick="filterByDate('all')">All</a></li>
              </ul>
            </div>
            </div>  
          </div>
        ...
        <div class="customer-table">
            ...
            <table class="table align-middle" id="transactionTable">
              ...
              <tbody>
                  ...
                  $sql = "SELECT transaction_id, payer_email, amount, currency, booking_date, invoice_path FROM payments ORDER BY booking_date DESC";
                  $result = $conn->query($sql);

                  if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                          $invoice_path = $row['invoice_path'];
                          $file_path = "../customer page/booking/invoices/" . basename($invoice_path);
                          
                          $download_link = ($invoice_path && file_exists($file_path))
                              ? "<a href='$file_path' target='_blank' class='btn-download'>View Invoice</a>"
                          ...
                          echo "<tr>
                                  ...
                                  <td>RM {$row['amount']}</td>
                                  <td class='booking-date'>{$row['booking_date']}</td>
                                  <td>$download_link</td>
                                </tr>";
                      }
                  } else {
...
function searchTransaction() {
    ...
    rows.forEach(row => {
        ...
        row.style.display = transactionID.includes(input) ? "" : "none";
    });
}
...
function filterByDate(range) {
    ...
    rows.forEach(row => {
        let dateText = row.querySelector(".booking-date").innerText;
        let bookingDate = new Date(dateText);
        let showRow = false;

        if (range === "today") {
            showRow = bookingDate.toDateString() === today.toDateString();
        } else if (range === "week") {
            let startOfWeek = new Date(today);
            startOfWeek.setDate(today.getDate() - today.getDay());
            showRow = bookingDate >= startOfWeek;
        } else if (range === "month") {
            showRow = bookingDate.getFullYear() === today.getFullYear() &&
