...
<!--end sidebar-->
      ...
      <!--breadcrumb-->
        ...
        <div class="row g-3">
          <div class="col-auto">
            <div class="position-relative">
              <input id="searchInput" class="form-control px-5" type="search" placeholder="Search Customers" onkeyup="searchCustomer()">
        ...
        <div class="customer-table">
            ...
            <?php
              ...
              $sql = "SELECT id, name, email, phone_number, profile_picture FROM users WHERE role = 'customer'";
                ...
                <table class="table align-middle" id="customerTable">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td>" . $counter . "</td>"; // Use counter instead of $row["id"]
                            echo "<td>";

                            // Profile picture next to name
                            if (!empty($row["profile_picture"])) {
                                echo '<img src="../customer page/profile/uploads/' . $row["profile_picture"] . '" alt="Profile" class="rounded-circle" width="40" height="40" style="object-fit: cover; margin-right: 10px;" />';
                            } else {
                                echo '<img src="../customer page/profile/default_profile_pic.webp" alt="No Image" class="rounded-circle" width="40" height="40" style="object-fit: cover; margin-right: 10px;" />';
                            }
                            ...
                            // Actions
                            echo '<td>
                                      <a href="backend/delete-user.php?id=' . $row["id"] . '" class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure?\');">Delete</a>
                                  </td>';
                            echo "</tr>";
  ...
  <script>
    ...
    // Get the table and exclude the last column
    let table = document.getElementById("customerTable");
    
    let data = [];
    let headers = [];
    
    // Extract headers except the last one (Actions)
    let headerCells = table.querySelectorAll("thead tr th");
    for (let i = 0; i < headerCells.length - 1; i++) {
        headers.push(headerCells[i].innerText);
    }
    
    // Extract row data except last column
    ...
    // Generate PDF Table
    doc.autoTable({
        head: [headers], // Set header
        body: data,      // Set body (excluding last column)
        startY: 20,
        theme: "grid",
        styles: { fontSize: 10 },
...
function searchCustomer() {
    ...
    for (let i = 1; i < rows.length; i++) { // Start from 1 to skip header row
        let nameColumn = rows[i].getElementsByTagName("td")[1]; // Name column index
        if (nameColumn) {
