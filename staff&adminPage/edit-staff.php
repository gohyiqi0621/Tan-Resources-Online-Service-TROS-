...
// Then, get the staff member's information
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $staff_name = $row['name'] ?? '';  // Default to an empty string if null
        $staff_email = $row['email'] ?? '';
        $staff_phone_number = $row['phone_number'] ?? '';
        $staff_role = $row['role'] ?? 'staff'; // Default to 'staff' if null
        $staff_profile_picture = !empty($row['profile_picture']) ? $row['profile_picture'] : 'default.jpg'; // Use default image if none exists
    } else {
    ...
    echo "<script>alert('❌ No staff ID provided or invalid ID!'); window.history.back();</script>";
    exit();
}
?>
  ...
  <main class="main-wrapper">
    <div class="main-content">
      ...
      <div class="items" data-group="test">
        <div class="card">
          <div class="card-body">
            <h2 class="font-weight-light text-center py-3">Edit Staff</h2>
            <div class="item-content">
              <form action="backend/update-staff.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?php echo $id; ?>">

                <div class="mb-3">
                  <label for="inputName3" class="form-label">Name</label>
                  <input type="text" class="form-control" id="inputName3" name="name" value="<?php echo htmlspecialchars($staff_name); ?>" required>
                </div>

                <div class="mb-3">
                  <label for="inputEmail3" class="form-label">Email</label>
                  <input type="email" class="form-control" id="inputEmail3" name="email" value="<?php echo htmlspecialchars($staff_email); ?>" required>
                </div>

                <div class="mb-3">
                  <label for="inputPhone3" class="form-label">Phone Number</label>
                  <input type="text" class="form-control" id="inputPhone3" name="phone_number" value="<?php echo htmlspecialchars($staff_phone_number); ?>" required>
                </div>

                <div class="mb-3">
                  <label for="inputRole3" class="form-label">Role</label>
                  <select class="form-control" id="inputRole3" name="role" required>
                    <option value="staff" <?php if ($staff_role == 'staff') echo 'selected'; ?>>Staff</option>
                    <option value="admin" <?php if ($staff_role == 'admin') echo 'selected'; ?>>Admin</option>
                  </select>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <?php 
                  $staff_image_path = "uploads/" . $staff_profile_picture;
                  if (!empty($staff_profile_picture) && file_exists($staff_image_path)) { ?>
                    <img src="<?php echo htmlspecialchars($staff_image_path); ?>" 
                         alt="Staff Profile Picture" width="100" height="100" 
                         style="border-radius: 50%;"><br>
                  <?php } else { ?>
                    <img src="user-profile-default.webp" 
                ...
                <button type="submit" name="edit_staff" class="btn btn-success">Edit Staff</button>
              </form>
            </div>
          </div>
        </div>
